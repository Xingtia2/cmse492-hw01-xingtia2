...
def build_region_revenue_share_table(sales_df: pd.DataFrame, customer_df: pd.DataFrame) -> pd.DataFrame:
    data = sales_df.copy()
    data["date"] = pd.to_datetime(data["date"], errors="coerce")
    data["revenue"] = data["units"] * data["unit_price"]
    ...
    total_revenue_all = summary["total_revenue"].sum()
    summary["avg_revenue_per_order"] = (summary["total_revenue"] / summary["orders"]).round(2)
    summary["revenue_share_pct"] = ((summary["total_revenue"] / total_revenue_all) * 100).round(2)
    summary["total_revenue"] = summary["total_revenue"].round(2)
